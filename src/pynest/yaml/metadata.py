"""
pynest.yaml.metadata
--------------------
Manages persistent registry metadata and file hashes/mtimes in .pynest/
"""
...
class MetadataManager:
    """
    Manages `.pynest/` storage for incremental scanning and registry persistence.
    """

    def __init__(self, project_root: Path) -> None:
        self.project_root = project_root.resolve()
        self.pynest_dir = self.project_root / ".pynest"
        self.index_file = self.pynest_dir / "yaml_index.json"
        self.meta_file = self.pynest_dir / "yaml_metadata.json"
    ...
    def _ensure_dir(self) -> None:
        if not self.pynest_dir.exists():
            self.pynest_dir.mkdir(parents=True, exist_ok=True)
